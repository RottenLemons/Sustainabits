<script setup lang="ts">
  ...
  import { users, friendChange, searchF } from "@/service/TalkToDB";
  ...
  let items = ref([{}]);
  onBeforeMount(async () => {
      let response = await users(user.userID);
  ...
  const query = ref('')
    ...
    let response = await searchF(user.userID, query.value);
    items.value = response.users
    });
</script>
...
<template>
  <main style="width: 100%;">
    <div class="flex flex-column align-items-center justify-content-center" style="backgroundColor: #337357; width: 100%; min-height: 94vh;">
        <p style="font-size: 4em; font-weight: bold; color: white; margin-bottom: 0.3em; margin-top: 0.5em;" class="font">Leaderboard</p>
        <DataView paginator :rows="5" :value="items" dataKey="userID" style="width: 70%; margin-bottom: 3em;">
            <template #header>
                            ...
                            <div class="flex flex-column md:flex-row justify-content-between md:align-items-center flex-1 gap-4">
                                ...
                                <div class="flex flex-column md:align-items-end gap-5">
                                    <div class="flex flex-column gap-2">
                                        <span class="text-xl font-semibold text-900" style="text-align: end;">{{ item.totalPoints }} points</span>
